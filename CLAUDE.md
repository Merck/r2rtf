...
This is an R package for generating RTF (Rich Text Format) tables,
figures, and listings. As a senior data scientist maintaining this
package, I should be familiar with RTF syntax and R package development
best practices.

## Key Information

- **Package Purpose**: Generate RTF tables, figures, and listings
  programmatically in R
- **Domain**: Clinical trial reporting and data visualization
- **RTF Expertise**: Understanding of RTF syntax and specification is
  essential
...
Required system libraries for development: - `libgit2` - for gert
package - `harfbuzz`, `fribidi` - for textshaping package - `freetype`,
`libpng`, `libtiff`, `libjpeg-turbo` - for ragg package - `pkg-config` -
for package configuration - `pandoc` - for building documentation and
vignettes

Install with:
`brew install libgit2 harfbuzz fribidi freetype libpng libtiff libjpeg-turbo pkg-config pandoc`
...
- **Issue \#235**: Added `text_hyphenation` parameter to
  [`rtf_title()`](https://merck.github.io/r2rtf/reference/rtf_title.md),
  [`rtf_colheader()`](https://merck.github.io/r2rtf/reference/rtf_colheader.md),
  and
  [`rtf_subline()`](https://merck.github.io/r2rtf/reference/rtf_subline.md)
