...
knitr::read_chunk("clinical-reporting-rounding.R")
```
...
Clinical statistics are calculated from unrounded values. At the final display
boundary, `format_fixed_ties_away()` applies ties-away-from-zero rounding and
then retains the requested number of decimal places with fixed-point formatting.

```{r clinical-reporting-rounding}
```
...
fmt_est <- function(data, columns = c("mean", "sd"), decimals = c(1, 2)) {
  .mean <- format_fixed_ties_away(data[[columns[[1]]]], decimals[1])
  if (length(columns) > 1) {
    .sd <- format_fixed_ties_away(data[[columns[[2]]]], decimals[2])
    paste0(.mean, " (", .sd, ")")
  } else {
    .mean
  }
...
fmt_ci <- function(data, columns = c("lower.CL", "upper.CL"), decimals = 2) {
  .lower <- format_fixed_ties_away(data[[columns[[1]]]], decimals)
  .upper <- format_fixed_ties_away(data[[columns[[2]]]], decimals)
  paste0("(", .lower, ", ", .upper, ")")
}
```
...
fmt_pval <- function(data, columns = "p.value", decimals = 3) {
  ...
  if_else(data[[columns[[1]]]] < scale, p_scale,
    format_fixed_ties_away(data[[columns[[1]]]], decimals)
  )
}
```
...
t3 <- data.frame(rmse = paste0(
  ...
  format_fixed_ties_away(sd(HAMD17_lmfit$residuals), 2)
))
```
