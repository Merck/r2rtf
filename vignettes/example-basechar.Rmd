...
bs_count <- function(data, grp, var,
  ...
  res <- res %>% mutate(pct = format_fixed_ties_away(n / sum(n) * 100, decimal))
...
bs_continuous <- function(data, grp, var,
  ...
  res <- data %>%
    ...
    summarise(
      ...
      Mean = format_fixed_ties_away(mean(var), decimal),
      SD = format_fixed_ties_away(sd(var), decimal),
      Median = format_fixed_ties_away(median(var), decimal),
