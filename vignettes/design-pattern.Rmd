...
resource_files:
   ...
   - pdf/pageby-disposition.pdf
   - pdf/pageby-firstrow-ae.pdf
   - pdf/para-indent.pdf
...
the first argument are all `tbl` except `rtf_read_png()` and `write_rtf()`.
...
Similar step is used to create a figure. First we generate a figure in `png` format, then use `rtf_read_png()` to read in the `png` file, and proceed to rtf generation.
...
If we save the RTF code into an `.rtf` file. Microsoft Word or other RTF Viewer software can display the file 
properly.
