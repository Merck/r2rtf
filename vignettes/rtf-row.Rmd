...
The numeric display example keeps calculations unrounded until display.
`format_fixed_ties_away()` then applies ties-away-from-zero rounding and retains
the requested number of decimal places with fixed-point formatting.
...
```{r}
db <- iris %>% mutate(Sepal.Length = format_fixed_ties_away(Sepal.Length * 2, 1))
