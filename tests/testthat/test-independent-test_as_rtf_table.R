context("Independent testing for as_rtf_table.R")

test_that("Case in defalut", {

  x <- iris[1:1,] %>% dplyr::select(1) %>% rtf_body() %>% as_rtf_table() %>% strsplit("\n")
  #x1 <- paste(x,  collapse = "\n")
...
test_that("Case for border_color_left and border_color_top", {
  x <- iris[1:1,] %>% dplyr::select(1) %>%
        rtf_body(border_color_left='red', border_color_top='blue') %>% as_rtf_table() %>% strsplit("\n")
