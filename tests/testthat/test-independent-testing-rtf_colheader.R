...
test_that("border color", {
  x <- r2rtf_tbl1 %>% rtf_body() %>% rtf_colheader(colheader =" a | b | c ", border_color_left   = "green")
  xx <- r2rtf_tbl1 %>% rtf_body() %>% rtf_colheader(colheader =" a | b | c ", border_color_left  = c("red", "green", "blue"))
  y <- r2rtf_tbl1 %>% rtf_body() %>% rtf_colheader(colheader = "  | b |   ", border_color_right  = c("red", "green", "blue"))
  z <- r2rtf_tbl1 %>% rtf_body() %>% rtf_colheader(colheader =" a |   |c  ", border_color_top    = c("blue", "green", "blue"))
  u <- r2rtf_tbl1 %>% rtf_body() %>% rtf_colheader(colheader =" a | b | c ", border_color_bottom = "yellow")
...
test_that("cell text formats", {
  x <- r2rtf_tbl1 %>% rtf_body() %>% rtf_colheader(colheader =" a | b | c ", text_format   = c("u", "i", "s"))
